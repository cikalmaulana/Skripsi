...
Kelas TypeScript akan digunakan oleh Angular di dalam sebuah komponen. Komponen pada Angular merupakan sebuah penyusun utama untuk aplikasi Angular. Setiap komponen yang ada pada aplikasi terdiri dari \textit{template} HTML untuk mendeklarasi halaman yang akan dimuat, kelas TypeScript, serta CSS Selector.
...
<ion-item>
	...
	\item List \\
	Komponen List terdiri dari beberapa baris \textit{item} yang dapat berisi teks, tombol, \textit{toggles}, ikon, thumbnails, dan komponen-komponen lainnya menggunakan \textit{tag} <ion-list> (Kode~\ref{lst:listComponent}). List mendukung untuk pengguna melakukan \textit{swiping}, \textit{dragging}, dan penghapusan \textit{item}. 
	
\begin{lstlisting}[language=php, label={lst:listComponent}, caption=Potongan Kode Program dari List Component]
<ion-list>
  <ion-item>
...
<ion-segment (ionChange)="segmentChanged($event)">
	...
	\item Slides \\
	Komponen Slides merupakan sebuah \textit{multi-section container} yang setiap bagiannya dapat dilakukan \textit{swipe} atau \textit{drag}. Untuk menggunakan komponen ini dengan menggunakan \textit{tag} <ion-slides> (Kode~\ref{lst:slidesComponent}).
	
	\begin{lstlisting}[language=php, label={lst:slidesComponent}, caption=Kode Program dari Slides]
...
<ion-tabs>
	...
	\begin{lstlisting}[language=php, label={lst:toastComponent}, caption=Kode Program dari Toast]
async presentToast() {
	...
	<ion-fab-list>
		<ion-fab-button>
			<ion-icon name="logo-facebook"></ion-icon>
				...
				\item Item \\
				Terdapat perbedaan pada \textit{tag} <ion-item>, dimana pada Ionic 3, \textit{tag} <ion-label> akan secara otomatis ditambahkan ke dalam <ion-item>. Sedangkan pada Ionic 4 diharuskan untuk menambahkan \textit{tag} <ion-label> secara manual ke dalam komponen item (Kode~\ref{lst:itemIonic4}). 
				
				\begin{lstlisting}[language=php, label={lst:itemIonic4}, caption=Penggunaan Item pada Ionic 4]
<ion-item>
	<ion-label>
    	Default Item
	</ion-label>
...
\newpage
				...
				\item Navbar \\
				Pada Ionic 4, terdapat penghapusan terhadap komponen <ion-navbar> karena untuk menjaga agar selalu menggunakan <ion-toolbar> dengan {\it back button} yang eksplisit (Kode~\ref{lst:navbarIonic4}).
...
<ion-toolbar>
	...
	<ion-title>My Navigation Bar</ion-title>
				...
				\begin{lstlisting}[language=php, label={lst:overlaysIonic4}, caption=Penggunaan Overlay untuk Toast pada Ionic 4]
async presentToast() {
    const toast = await this.toastController.create({
      message: 'Your settings have been saved.',
      duration: 2000
    ...
    toast.present();
				...
				\item Scroll \\
				\textit{Tag} <ion-scroll> sudah dihapus pada Ionic 4 agar penggunaan \textit{tag} <ion-content> menjadi lebih maksimal.
				
				\item Segment Button \\
				Pada Ionic 4, teks pada Segment Button membutuhkan <ion-label> untuk membungkusnya (Kode~\ref{lst:segmentButtonIonic4}).
				\begin{lstlisting}[language=php, label={lst:segmentButtonIonic4}, caption=Penggunaan Segment Button untuk Toast pada Ionic 4]
<ion-segment-button>
...
\begin{lstlisting}[language=php, label={lst:updateAngular}, caption=Kode untuk Memperbarui Versi Ionic 6 dengan versi Angular Terbaru]
...
\begin{lstlisting}[language=php, label={lst:updateModal}, caption=Kode ion-modal menggunakan Shadow DOM pada CSS]
...
\begin{lstlisting}[language=php, label={lst:updatePopover}, caption=Kode ion-popover menggunakan Shadow DOM pada CSS]
